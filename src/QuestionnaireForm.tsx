...
export default function QuestionnaireForm({ steps, initialAnswers, onSaveProgress, onComplete, darkMode }: Props) {
    ...
    const [currentIdx, setCurrentIdx] = useState(() => { if (initialAnswers) { for (let i = 0; i < steps.length; i++) if (!initialAnswers[steps[i].id]) return i; return steps.length } return 0 })
    const [answers, setAnswers] = useState<Record<string, string>>(initialAnswers || {})
    const [customText, setCustomText] = useState('')
    const [preSelected] = useState<Record<string, string>>(() => { const init: Record<string, string> = {}; for (const s of steps) { if (initialAnswers?.[s.id]) init[s.id] = initialAnswers[s.id]; else if (s.default) init[s.id] = s.default } return init })
    useEffect(() => { if (currentIdx >= total) setShowReview(true) }, [])
    const saveProgress = (a: Record<string, string>) => onSaveProgress?.(a)
    const step = currentIdx < total ? steps[currentIdx] : null
